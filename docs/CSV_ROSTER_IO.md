...
iOS has **no shared Downloads folder an app may write to**. The nearest
equivalent is the app's own Documents directory surfaced in the Files app, so
the export writes there — `Files → On My iPhone → CricSim → player-names.csv` —
and the app sets `UIFileSharingEnabled` and `LSSupportsOpeningDocumentsInPlace`
(via the `expo-file-system` config plugin) to make that directory visible.

Documents rather than cache, because iOS reclaims cache under storage pressure
and a file the user was told about must still be there when they go looking.

The share sheet is then offered on top, for Save to Files → Downloads, AirDrop
or mail. It is deliberately **not** load-bearing: the file is already on disk
before it opens, so a device without sharing still gets a real export rather
than an error.

### Import

Restricted to `type: 'text/csv'`. On iOS 14+ expo-document-picker maps that
through `UTType(mimeType:)` to the system UTI
`public.comma-separated-values-text`, so every real `.csv` stays selectable and
everything else is greyed out. Note that unresolvable MIME types are silently
dropped from the allowed list, so a typo'd type would grey out *everything* —
worth re-checking if the picker ever looks empty.

### Presentation

Results and instructions are native `Alert`s fired after each action, not
standing text in the sheet. The account sheet is 280pt wide; a paragraph of
permanent help crowded the sections around it for a feature most users touch
once. The buttons are stacked full-width for the same reason — side by side
they got ~80pt of text width and rendered as "Export na…".
...
**Adding either module — or changing the `expo-file-system` plugin options —
requires `make mobile-prebuild`** followed by an Xcode rebuild. A JS reload will
not pick it up, and neither will a rebuild alone, since `Info.plist` is
regenerated by prebuild (`ios/` is gitignored).
